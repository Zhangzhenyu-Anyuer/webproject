            ...
            <div class="content">
                ...
                <ul class="nav full-left">
                    <li v-for="(header,index) in header_list" :key="index">
                        <span>
                        <a :href="header.link" v-if="header.is_site">{{ header.title }}</a>
                        <router-link :to="header.link" v-else>{{ header.title }}</router-link>
                        </span>
                    </li>
                </ul>
                <div class="login-bar full-right">
                    ...
                    <div class="login-box full-left" v-if="token">
                        ...
                        &nbsp;|&nbsp;
                        <span @click="quit_login">退出登录</span>
                    </div>
...
export default {
    ...
    methods: {
            ...
            this.$axios.get(this.$settings.HOST + 'home/nav/').then(response => {
                for (let n = 0; n < response.data.length; n++) {
                    if (response.data[n].position === 1) {
                        this.header_list.push(response.data[n])
                    }
                }
            })
        },
        ...
        get_course_length(){
            ...
            if (token){
                this.$axios({
                    url: this.$settings.HOST+'cart/cart_len/',
                    method:"get",
                    headers: {
                    ...
                    this.$store.state.cart_length = res.data.cart_length
                })
            }
        },
    },
