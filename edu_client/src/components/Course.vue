        ...
        <div class="main">
            ...
            <div class="condition">
                <ul class="cate-list">
                    ...
                    <li v-for="(cate,index) in category_list" @click="category=cate.id"
                        :class="category===cate.id ? 'this': ''">{{ cate.name }}
                    </li>
            ...
            <div class="course-list">
                <div class="course-item" v-for="(course,index) in course_list" :key="index">
                    ...
                    <div class="course-info">
                        <h3><router-link :to="'detail/'+course.id">
                            {{ course.name }} <span><img src="/static/image/avatar1.svg" alt="">{{
                                course.students
                            }}人已加入学习</span></router-link></h3>
                        <p class="teather-info">{{ course.teacher.title }} <span>共{{
                                course.pub_lessons
                            }}课时/{{
                                course.lessons === course.pub_lessons ? '更新完成' : `已更新${course.pub_lessons}`
                            }}</span></p>
                        <ul class="lesson-list">
                            <li v-for="(lesson,index) in course.lesson_list"><span class="lesson-title">{{ index + 1 }} | 第{{
                                    index + 1
                                }}节：{{
                                    lesson.name
                                }}</span>
...
export default {
    ...
    methods: {
        ...
        change_order_class(type) {
            if (this.filters.type === type && this.filters.order === 'asc') {
                return 'this asc'
            } else if (this.filters.type === type && this.filters.order === 'desc') {
                return 'this desc'
            } else {
                return ''
            }
        ...
        get_all_course() {
            ...
            this.$axios.get(this.$settings.HOST + 'course/course/', {params: filters}).then(res => {
