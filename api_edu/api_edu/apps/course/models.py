...
class Course(BaseModel):
    ...
    def get_active_list(self):
        active_list = self.activeprices.filter(is_delete=False, is_show=True,
    ...
    def discount_name(self):
        ...
        print(self.name, '课程参与了', active_list, '活动')
        ...
        if active_list:
            ...
            if self.price > float(condition):
                ...
                if sale == '':
                    ...
                    # 说明是满减活动
                    """ 500-80  400-40 300-20 100-10  """
                    split = sale.split('\r\n')
                    sale_list = []
                    for item in split:
                        sale_item = item[1:]
                        # 门槛价  优惠价
                        condition_price, sale_price = sale_item.split('-')
                        if self.price > float(condition_price):
                            sale_list.append(float(sale_price))
                    if sale_list:
    ...
    def active_time(self):
        ...
        # 活动剩余的时间 = 活动结束的时间 - 现在的时间
        return int(time)

    @property
    def expire_time(self):
        expire_filter = self.course_expire.filter(is_show=True, is_delete=False)
        data = []
        for item in expire_filter:
            data.append({
                'id': item.id,
